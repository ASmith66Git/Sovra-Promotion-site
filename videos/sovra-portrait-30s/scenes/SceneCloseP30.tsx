...
export const SceneCloseP30: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  const containerOp  = clampedInterpolate(frame, [0, 24], [0, 1]);
  const logoSpring   = spring({ frame: frame - 6,  fps, config: { damping: 10, stiffness: 70 } });
  const tag1Op       = delayedInterpolate(frame, 0.8, 0.6, 0, 1);
  const tag1Y        = delayedInterpolate(frame, 0.8, 0.6, 25, 0);
  const tag2Op       = delayedInterpolate(frame, 1.2, 0.6, 0, 1);
  const tag2Y        = delayedInterpolate(frame, 1.2, 0.6, 25, 0);
  const badgeSpring  = spring({ frame: frame - 80, fps, config: { damping: 10, stiffness: 70 } });
  ...
  return (
    <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 8cqw", zIndex: 20, opacity: containerOp }}>
      ...
      {/* Taglines */}
      <p style={{ fontSize: "7cqw", fontWeight: 800, color: "#F8FAFC", textAlign: "center", lineHeight: 1.2, marginBottom: "1.5cqh", opacity: tag1Op, transform: `translateY(${tag1Y}px)` }}>
        Stop organising.
      </p>
      <p style={{ fontSize: "7cqw", fontWeight: 800, textAlign: "center", background: "linear-gradient(90deg, #818CF8, #C084FC, #818CF8)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", lineHeight: 1.2, marginBottom: "8cqh", opacity: tag2Op, transform: `translateY(${tag2Y}px)` }}>
        Start living.
      </p>
