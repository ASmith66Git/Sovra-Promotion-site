    ...
    >
      {ICON_SEEDS.map((seed, i) => {
        ...
        return (
          ...
          >
            <Icon size={`${seed.size * 1.4}cqw`} />
          </div>
        );
      })}

      <div
        className="text-center z-10 bg-slate-900/65 px-[5cqw] py-[4cqh] rounded-3xl backdrop-blur-md border border-white/10 max-w-[70cqw]"
        style={{
          opacity: cardOpacity,
          transform: `scale(${0.85 + 0.15 * cardSpring})`,
        }}
      >
        <p
          className="text-[3cqw] text-indigo-300 font-semibold uppercase tracking-[0.25cqw] mb-[2cqh]"
          style={{ opacity: line1Opacity, transform: `translateY(${line1Y}px)` }}
        >
          Information finds you everywhere
        </p>
        <h1
          className="text-[6.5cqw] font-bold text-white leading-tight tracking-tight"
          style={{ opacity: line1Opacity, transform: `translateY(${line1Y}px)` }}
        >
          Saving it?
          <br />
        ...
        <p
          className="mt-[2cqh] text-[2.5cqw] text-slate-400 font-medium"
